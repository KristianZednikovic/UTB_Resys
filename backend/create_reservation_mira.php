...
try {
    // Check for duplicate email in reservations_mira table
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM reservations_mira WHERE email = ?");
    ...
    // Check for duplicate team name in reservations_mira table
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM reservations_mira WHERE team_name = ?");
    ...
    // Check for time slot availability in reservations_mira table
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM reservations_mira WHERE time_slot = ?");
    ...
    $stmt = $pdo->prepare("
        INSERT INTO reservations_mira (team_name, team_number, email, time_slot)
        VALUES (?, ?, ?, ?)
    ");
    $stmt->execute([
        $input['team_name'],
    ...
    $stmt = $pdo->prepare("SELECT * FROM reservations_mira WHERE id = ?");
