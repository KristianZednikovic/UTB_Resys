...
try {
    ...
    // Check if the new time slot is available in reservations_mira table
    $checkStmt = $pdo->prepare("SELECT COUNT(*) FROM reservations_mira WHERE time_slot = ? AND id != ?");
    ...
    $getStmt = $pdo->prepare("SELECT * FROM reservations_mira WHERE id = ?");
    ...
    $updateStmt = $pdo->prepare("UPDATE reservations_mira SET time_slot = ? WHERE id = ?");
    ...
    error_log("Database error in time_change_mira.php: " . $e->getMessage());
    sendErrorResponse('Database error occurred', 500);
} catch (Exception $e) {
    if ($pdo->inTransaction()) {
        $pdo->rollBack();
    }
    error_log("Error in time_change_mira.php: " . $e->getMessage());
